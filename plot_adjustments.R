...
put.fig.letter("G.", location = "topleft", offset=c(0.05, -0.05), cex=1.5)
...
suppressWarnings(pv<-wilcox.test(tmp1$res, tmp2$res, paired=T)$p.value)
...
plot(ag_old$est[,1], ag_old$obs[,1], pch=c(18,15,15)[as.numeric(as.factor(ag_old$sp))],
     ...
     ylab="",
     type="n")
put.fig.letter("H.", location = "topleft", offset=c(0.05, -0.05), cex=1.5)
...
plot(ag_new$est[,1], ag_new$obs[,1], pch=c(18,15,15)[as.numeric(as.factor(ag_new$sp))],
     ...
     ylab="",
     type="n")
put.fig.letter("I.", location = "topleft", offset=c(0.05, -0.05), cex=1.5)
